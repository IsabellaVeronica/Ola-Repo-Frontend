...
import { 
  FileText, Loader2, FileDown, TrendingUp, 
...
export const SalesProfitReports = () => {
    ...
    const loadOrderDetails = async (order: any) => {
        ...
        try {
            const res = await FetchData<any>(`/api/ventas/${order.id_venta}`);
            if (res) {
                setOrderDetails(res);
            }
        } catch (error) {
    ...
    const handleDownloadCSV = () => {
        ...
        setExporting('csv');
        try {
            const headers = ['ID Venta', 'ID Pedido', 'Cliente', 'Fecha', 'Origen', 'Ingreso (USD)', 'Costo Prod. (USD)', 'Ganancia (USD)', 'Moneda Pago', 'Monto Pago Real'];
            ...
            link.setAttribute('download', `reporte_ventas_ganancias_${new Date().toISOString().split('T')[0]}.csv`);
    ...
    const handleDownloadPDF = () => {
        ...
        try {
            ...
            doc.setTextColor(80);
            doc.text(`Rango: ${fromDate} hasta ${toDate}`, 14, 27);
            doc.text(`Generado: ${new Date().toLocaleString()}`, 14, 33);

            // KPIs Grid inside PDF
            doc.setFillColor(245, 247, 250);
            doc.rect(14, 39, 182, 28, 'F');
            
            doc.setFontSize(8);
            doc.setTextColor(100);
            doc.text('INGRESOS (VENTAS)', 18, 46);
            doc.text('COSTO DE VENTAS', 58, 46);
            doc.text('GANANCIA BRUTA', 98, 46);
            doc.text('GASTOS', 138, 46);
            doc.text('UTILIDAD NETA', 170, 46);

            doc.setFontSize(11);
            doc.setFont('helvetica', 'bold');
            doc.setTextColor(30, 41, 59); // dark slate
            doc.text(`$${kpiData.total_ingresos.toLocaleString()}`, 18, 56);
            doc.text(`$${kpiData.total_costo.toLocaleString()}`, 58, 56);
            
            doc.setTextColor(16, 185, 129); // green
            doc.text(`$${kpiData.ganancia_bruta.toLocaleString()}`, 98, 56);
            
            doc.setTextColor(244, 63, 94); // rose
            doc.text(`$${kpiData.total_gastos.toLocaleString()}`, 138, 56);
            ...
            doc.text(`$${kpiData.ganancia_neta.toLocaleString()}`, 170, 56);

            // Sales Table
            const head = [['ID Venta', 'ID Pedido', 'Cliente', 'Fecha', 'Origen', 'Ingresos', 'Costos', 'Ganancia']];
            ...
            doc.save(`reporte_ventas_ganancias_${new Date().toISOString().split('T')[0]}.pdf`);
        } catch (error) {
                    ...
                    <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-6">
                        <div className="space-y-1">
                            <h2 className="text-xl font-bold text-foreground flex items-center gap-2">
                                <TrendingUp className="h-5 w-5 text-primary animate-pulse" /> Reporte de Rentabilidad
                            </h2>
                ...
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-in fade-in duration-200">
                    <Card className="w-full max-w-2xl bg-card border border-border shadow-2xl overflow-hidden rounded-2xl">
                        <CardHeader className="border-b border-border pb-4 flex flex-row justify-between items-center space-y-0">
                            <div>
                                <CardTitle className="text-md font-bold text-foreground">Desglose de Venta #{selectedOrder.id_venta}</CardTitle>
                                <CardDescription className="text-xs mt-1">
                                    Venta ID: <span className="font-semibold text-foreground">#{selectedOrder.id_venta}</span>
                                    {selectedOrder.id_pedido ? ` | Pedido ID: #${selectedOrder.id_pedido}` : ''}
                                    {` | Cliente: `}<span className="font-semibold text-foreground">{selectedOrder.cliente_nombre || 'Invitado'}</span> | Fecha: {new Date(selectedOrder.fecha).toLocaleString()}
                                </CardDescription>
                            </div>
                                    ...
                                    <table className="w-full text-left text-xs border-collapse">
                                        ...
                                        <tbody className="divide-y divide-border/60">
                                            {orderDetails?.items?.map((item: any) => {
                                                ...
                                                return (
                                                    <tr key={item.id_venta_item || item.id_pedido_item || item.id_variante_producto} className="hover:bg-muted/5 font-medium">
