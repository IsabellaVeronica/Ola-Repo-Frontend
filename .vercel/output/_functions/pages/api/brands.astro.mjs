...
const ALL = async ({ request }) => {
  ...
  const token = authHeader?.startsWith("Bearer ") ? authHeader.split(" ")[1] : request.headers.get("cookie")?.split("; ").find((row) => row.startsWith("token="))?.split("=")[1];
  ...
  try {
    ...
    const targetUrl = `${externalApiBase}/brands${searchParams}`;
